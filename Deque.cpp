...
public:
    ...
    friend std::ostream &operator<<(std::ostream &os, const Deque &lst);
};
...
void Deque::deleteNode(Node *cur)
{
    if (cur == head)
    {
        pop_front();
...
std::ostream &operator<<(std::ostream &os, const Deque &lst)
{
    Node *cur = lst.head;
    while (cur)
    {
